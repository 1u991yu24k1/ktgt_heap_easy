...
void so_useful(void){
    unsigned long addr_puts = (unsigned long)puts;
    printf("[+] address of `puts`:  0x%lx\n", addr_puts);
    return ;
}
...
void edit_note(void){
    ...
    idx = (int)read_int();
    if(0 > idx || MAX_SIZE <= idx || ptr_array[idx] == NULL){
        fputs("[!] Invalid index\n", stderr);
        exit(EXIT_FAILURE);
    }
    note = ptr_array[idx];
    size = (*(unsigned long *)((unsigned long)note - 0x8uL)) & ~ 3ul; 
    fputs("[*] Note data: ",stdout);
    readnum = read(fileno(stdin), note, size - 0x10 - 1);
    if(readnum <= 0){
